...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'GET') {
        ...
        try {
            ...
            // Calculate price for each seat based on row
            $priceStmt = $pdo->prepare("SELECT price_high, price_mid, price_low FROM shows WHERE id = ?");
            $priceStmt->execute([$showId]);
            $prices = $priceStmt->fetch();
            
            // Insert bookings
            $bookingIds = [];
            foreach ($seats as $seat) {
                // Determine price based on row
                $price = $prices['price_low']; // Default
                if ($seat['row'] <= 3) {
                    $price = $prices['price_high'];
                } elseif ($seat['row'] <= 7) {
                    $price = $prices['price_mid'];
                }
            ...
            echo json_encode([
                'success' => true,
                'bookingId' => $bookingIds[0], // Return first booking ID
